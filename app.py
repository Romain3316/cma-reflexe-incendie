...
def commune_coordinates(commune: str) -> tuple[float, float] | None:
    commune_key = (commune or "").strip().casefold()
    for item in COMMUNES_INCENDIE:
        if item.get("commune", "").strip().casefold() == commune_key:
            return float(item["lat"]), float(item["lon"])
    # Les trois communes de permanence sont garanties, même si la cartographie évolue.
    for item in PERMANENCES_BUS_ARTISANAT:
        if item["commune"].strip().casefold() == commune_key:
            return float(item["lat"]), float(item["lon"])
    return None
...
def draw_bus_cover_banner(pdf: canvas.Canvas, commune_entreprise: str) -> None:
    page_w, page_h = A4
    x, y, w, h = 44, page_h - 318, page_w - 88, 58
    pdf.setFillColor(HexColor("#F7F9FC"))
    pdf.roundRect(x, y, w, h, 10, stroke=0, fill=1)
    pdf.setFillColor(HexColor(CMA_RED))
    pdf.roundRect(x, y, 7, h, 3, stroke=0, fill=1)

    logo = get_bus_logo_reader()
    if logo:
        pdf.drawImage(logo, x + 13, y + 8, width=86, height=42, preserveAspectRatio=True, mask="auto")

    text_x = x + 108
    pdf.setFillColor(HexColor(CMA_BLUE))
    pdf.setFont("Helvetica-Bold", 10.5)
    pdf.drawString(text_x, y + 38, "LE BUS DE L'ARTISANAT VIENT À VOTRE RENCONTRE")
    pdf.setFillColor(HexColor(CMA_TEXT))
    pdf.setFont("Helvetica", 8.6)
    pdf.drawString(text_x, y + 23, "3 permanences CMA sont organisées cette semaine : Le Porge, Lège-Cap-Ferret et Mios.")
    if commune_entreprise:
        pdf.setFillColor(HexColor(CMA_MUTED))
        pdf.setFont("Helvetica-Oblique", 7.8)
        pdf.drawString(text_x, y + 10, "Les distances depuis votre commune figurent dans le guide.")
...
def draw_cover(
    ...
    draw_bus_cover_banner(pdf, commune_entreprise)
...
def generate_pdf(
    ...
    # Page dédiée aux permanences du Bus de l'Artisanat.
    draw_bus_permanences_page(pdf, commune_entreprise, page_number)
    pdf.showPage()
    page_number += 1
...
with col3:
    ...
    commune_entreprise = st.selectbox(
        "Commune de l'établissement *",
        options=communes_options,
        format_func=lambda value: value or "Sélectionner une commune",
        help="Cette information permet d'afficher les distances jusqu'aux permanences du Bus de l'Artisanat.",
    )
...
if not commune_entreprise:
    st.warning("Renseignez la commune de l’établissement pour générer le PDF et calculer les distances jusqu’aux permanences.")
    st.stop()
