...
BANQUES_INCENDIES = [
    ...
    },
    {
        ...
        "bordure": "#E7A5A8",
        "mesures": [
            "Avance de trésorerie mobilisable rapidement jusqu'à 15 000 €.",
            "Possibilité de reporter les échéances de prêts professionnels.",
            "Étude au cas par cas avec le conseiller ; conditions précisées en agence.",
        ],
        "source": "Communiqué officiel Crédit Mutuel Arkéa du 30 juillet 2026.",
        "url": CREDIT_MUTUEL_SUD_OUEST_INCENDIES_URL,
    },
]
...
def draw_bank_page(
    ...
    # Bandeau synthétique à conserver sous les six cartes.
    strip_y = start_y - card_h * 3 - gap * 2 - 62
    strip_h = 50
    pdf.setFillColor(HexColor("#FFF7E8"))
    pdf.setStrokeColor(HexColor("#F0D7A5"))
    pdf.roundRect(margin, strip_y, content_w, strip_h, 8, stroke=1, fill=1)
    pdf.setFillColor(HexColor(CMA_BLUE))
    pdf.setFont("Helvetica-Bold", 8.5)
    pdf.drawString(margin + 12, strip_y + strip_h - 17, "À DEMANDER AU CONSEILLER BANCAIRE")
    draw_wrapped(
        pdf,
        (
            "Report ou suspension des échéances • crédit-bail • prêt de trésorerie "
            "d'urgence • facilité de caisse • confirmation écrite des coûts et conditions."
        ),
        margin + 12,
        strip_y + strip_h - 31,
        content_w - 24,
