...
def draw_cover(
    ...
    # Priorité post-incendies : synthèse URSSAF mise en avant sur la couverture.
    card_x, card_y = margin, 344
    card_w, card_h = page_w - 2 * margin, 244
    pdf.setFillColor(white)
    pdf.roundRect(card_x, card_y, card_w, card_h, 11, stroke=0, fill=1)
    pdf.setFillColor(HexColor(CMA_RED))
    pdf.roundRect(card_x, card_y, 8, card_h, 4, stroke=0, fill=1)

    pdf.setFillColor(HexColor(CMA_BLUE))
    pdf.setFont("Helvetica-Bold", 15)
    pdf.drawString(card_x + 22, card_y + card_h - 34, "PRIORITÉ POST-INCENDIES : URSSAF")
    pdf.setFillColor(HexColor(CMA_MUTED))
    pdf.setFont("Helvetica", 8.4)
    pdf.drawString(card_x + 22, card_y + card_h - 51, "Report des cotisations sociales · septembre, octobre et novembre 2026")

    blocks = [
        ("EMPLOYEURS", "Report possible des cotisations patronales sans demande préalable et sans pénalité. Les déclarations restent obligatoires. Cotisations salariales, CSG, CRDS et prélèvement à la source restent dus."),
        ("INDÉPENDANTS / MICRO", "Report possible sur demande via la messagerie sécurisée, motif « Demander moratoire incendie ». Les obligations déclaratives doivent rester à jour."),
        ("À SURVEILLER", "Une exonération distincte est annoncée, mais ses critères seront définis ultérieurement dans le PLFSS 2027. Le report ne donne pas automatiquement droit à l'exonération."),
        ("CPSTI", "Aide d'urgence jusqu'à 2 000 € pour les indépendants éligibles ; jusqu'à 8 000 € en cas de destruction totale des locaux professionnels et/ou de l'habitation personnelle, sous conditions."),
    ]
    col_gap = 14
    col_w = (card_w - 44 - col_gap) / 2
    block_h = 76
    for idx, (title, body) in enumerate(blocks):
        col = idx % 2
        row = idx // 2
        bx = card_x + 22 + col * (col_w + col_gap)
        by = card_y + 112 - row * 88
        pdf.setFillColor(HexColor("#F4F7FA"))
        pdf.roundRect(bx, by, col_w, block_h, 7, stroke=0, fill=1)
        pdf.setFillColor(HexColor(CMA_BLUE if idx != 2 else CMA_AMBER))
        pdf.setFont("Helvetica-Bold", 8.5)
        pdf.drawString(bx + 10, by + block_h - 17, title)
        draw_wrapped(pdf, body, bx + 10, by + block_h - 31, col_w - 20,
                     font_size=6.9, leading=8.4, color=CMA_TEXT, max_lines=6)
...
if not commune_entreprise:
    st.info(
        "Le PDF peut être généré sans commune. Certaines recommandations liées à la zone concernée "
        "devront alors être vérifiées manuellement avec l'entreprise."
    )
